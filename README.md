...
- **Collection is incremental and deduplicated.** One profile Actor run and one batched post Actor run cover all confirmed staff. The company account uses its own lightweight profile result and merges owner-verified `latestPosts`, avoiding a second post run full of tagged/collaborator rows. New rows are merged with the last validated 45-day cache and deduplicated by immutable post identity.
