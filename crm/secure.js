(() => {
  ...
  function clear(){clearTimeout(expiryTimer);sessionStorage.removeItem(key);window.dispatchEvent(new Event('kirpa-crm-locked'));}
  function expireAt(time){clearTimeout(expiryTimer);expiryTimer=setTimeout(clear,Math.max(0,time-Date.now()));}
  ...
  async function login(password){const s=await request('/session',{method:'POST',body:JSON.stringify({password})});sessionStorage.setItem(key,JSON.stringify(s));expireAt(s.expires);window.dispatchEvent(new Event('kirpa-crm-unlocked'));return true;}
  ...
  const current=session();if(current)expireAt(current.expires);
  document.addEventListener('visibilitychange',()=>{if(!document.hidden&&!session())clear();});
})();
